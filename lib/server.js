...
export async function readJson(pathname, fallback) {
  try {
    ...
    /* cache-buster: מונע ש-Vercel Blob CDN יחזיר גרסה ישנה מיד אחרי writeJson */
    const sep = b.url.includes('?') ? '&' : '?';
    const res = await fetch(b.url + sep + '_ts=' + (b.uploadedAt ? new Date(b.uploadedAt).getTime() : Date.now()), { cache: 'no-store' });
