...
export async function toWebmVideo(file, opts = {}) {
  ...
  try {
    ...
    /* מנסים להפעיל את הוידאו לפני הכל — אם autoplay חסום, נופלים לקובץ מקורי */
    try {
      await video.play();
    } catch (playErr) {
      console.warn('[toWebmVideo] video.play נכשל — כנראה autoplay חסום. מעלים את המקורי.', playErr);
      URL.revokeObjectURL(url);
      return file;
    }

    /* מחכים לפריים הראשון של הוידאו לפני שמתחילים לצייר,
       כדי שהקנבס יכיל תוכן אמיתי ולא רק שחור. */
    await new Promise((resolve) => {
      let done = false;
      const finish = () => { if (!done) { done = true; resolve(); } };
      if (video.readyState >= 2 && video.currentTime > 0) return finish();
      const check = () => {
        if (video.currentTime > 0 || video.readyState >= 3) finish();
        else if (!done) requestAnimationFrame(check);
      };
      video.addEventListener('playing', check, { once: true });
      requestAnimationFrame(check);
      setTimeout(finish, 500); /* fallback: מתחילים בכל מקרה אחרי חצי שניה */
    });

    /* מציירים פריים אחד ראשון עם התוכן — לא שחור */
    ...
    const drawLoop = () => {
      ...
      /* מציירים תמיד — גם אם paused זמנית (buffering) — כדי לא לאבד פריימים */
      if (video.videoWidth > 0) ctx.drawImage(video, 0, 0, w, h);
