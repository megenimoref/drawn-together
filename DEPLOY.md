...
- **שני סוגי קישורים:**
  - **קישור פרטי (עריכה):** `‎/c/<space>?edit=<token>‎` — הבעלים. מאפשר הוספה/שיבוץ/מחיקה
    וגישה לפרטי ההורה. ה-`editToken` נשמר גם ב-localStorage אחרי כניסה ראשונה.
  - **קישור שיתוף (צפייה בלבד):** `‎/c/<space>‎` (בלי טוקן) — מציג רק את הלוח, בלי כפתורי עריכה
    ובלי PII. השרת חוסם כל פעולת עריכה/קריאת-הגשות בלי הטוקן (401).
- **שחזור קישור אבוד:** רשימת הלוחות נשמרת ב-localStorage של המכשיר (עמוד הבית),
  ובנוסף מי שהשאיר מייל ביצירה יכול לקבל את הקישור **הפרטי** שוב דרך `/recover` (דורש הגדרת מייל, ראו למטה).
- **פרטיות:** הקישורים אקראיים וארוכים, ו-`/c/` חסום לאינדוקס ב-robots.txt.
